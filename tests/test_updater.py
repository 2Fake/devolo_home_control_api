...
class TestUpdater:
    ...
    def test_hasattr(self):
        updater_functions = (func for func in dir(self.homecontrol.updater)
                             if callable(getattr(self.homecontrol.updater,
    ...
    def test__binary_sync(self):
        uid = self.devices['blinds']['uid']
        self.homecontrol.updater._binary_sync(message={
            "properties": {
                "uid": f"bss.{uid}",
    ...
    def test__binary_switch(self):
        ...
        self.homecontrol.updater._binary_switch(message={
            "properties": {
    ...
    def test__grouping(self):
        ...
        self.homecontrol.updater._grouping(message={
            "properties": {
                "property.value.new": [{
                    "id": zone_id,
    ...
    def test__gui_enabled(self):
        ...
        self.homecontrol.updater._gui_enabled(message={
            "uid": element_uids[0],
    ...
    def test__multi_level_switch(self):
        ...
        uid = device.get("uid")
        self.homecontrol.devices.get(uid).multi_level_switch_property.get(device.get("elementUIDs")[0]).value = \
            device.get("value")
    ...
    def test__protection_local(self):
        ...
        self.homecontrol.updater._protection(message={
            "properties": {
                "uid": f"ps.{uid}",
    ...
    def test__voltage_multi_level_sensor(self):
        ...
        current = self.homecontrol.devices.get(uid).multi_level_sensor_property.get(
            f"devolo.VoltageMultiLevelSensor:{uid}").value
        self.homecontrol.updater._voltage_multi_level_sensor(
            message={"properties": {
                "uid": f"devolo.VoltageMultiLevelSensor:{uid}",
                "property.value.new": 234
            }})
        current_new = self.homecontrol.devices.get(uid).multi_level_sensor_property.get(
            f"devolo.VoltageMultiLevelSensor:{uid}").value
